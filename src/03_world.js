...
const BEVEL = 0.13;                                   // fillet radius as a fraction of the smallest side
...
function boxGeo(w, h, d, rr) {
  const r = rr === undefined ? Math.min(Math.min(w, h, d) * BEVEL, 0.08) : rr;
...
class Rig {
  constructor(variant = 'white', model = 'boy') {
    ...
    const head = new THREE.Mesh(boxGeo(0.36, 0.4, 0.36, 0.05), [skinM, skinM, skinM, skinM, skinFace, skinM]); head.position.y = 0.24; head.castShadow = true; neck.add(head);
