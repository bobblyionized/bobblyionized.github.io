...
class Rig {
  constructor(variant = 'white', model = 'boy') {
    ...
    const armM = dealer || tux || wd ? jm.sleeve : SKIN;
    const legM = dealer || tux ? jm.shorts : SKIN;
    ...
    for (const [n, sx] of [['L', 1], ['R', -1]]) {
      ...
      this['hand' + n] = box(0.13, 0.13, 0.09, skinM, 0, -0.355, 0.01, el);                          // hand
...
function makePalm(parent, ambient, x, z, h = 5) {
  ...
  for (let i = 0; i < segs; i++) {
    ...
    cyl(lerp(0.30, 0.16, t1), lerp(0.30, 0.16, t0), sh * 1.06, M.trunk, 0, sh / 2, 0, seg, 9);
    const br = lerp(0.335, 0.185, t0);
    cyl(br, br * 0.94, 0.13, M.bead, 0, 0.05, 0, seg, 9).castShadow = i < 3;                  // the bead at each joint
    if (i) cyl(br * 1.02, br * 1.02, 0.035, M.ring, 0, 0.13, 0, seg, 9).castShadow = false;   // dark growth ring on top of it
  }
  ...
  for (let i = 0; i < 10; i++) frond(i, 10, -0.15 - Math.random() * 0.1, 1, Math.random() * 0.2, 3);        // outer ring, the long drooping ones
  for (let i = 0; i < 6; i++) frond(i, 6, 0.46 + Math.random() * 0.14, 0.62, 0.32 + Math.random() * 0.2, 2);  // shorter inner ring angled up, so the crown reads full rather than spidery
...
function buildLobby() {
  ...
  const sea = new THREE.Mesh(new THREE.PlaneGeometry(400, 170, 170, 74), waterM);
...
function buildBeachCourt() {
  ...
  const water = new THREE.Mesh(new THREE.PlaneGeometry(160, 300, 68, 128), makeWaterMaterial([1, 0, -50])); water.rotation.x = -Math.PI / 2; water.position.set(50 + 80, -0.015, 0); beachCourt.add(water);   // sea along the +x side, from the shore line at x = 50 outward
