...
async function boot(online) {
  ...
  { let sp = 'auto'; try { sp = localStorage.getItem('vg_shaders') || 'auto'; } catch (e) { } $('#shSel').value = sp; $('#shNote').textContent = 'Running: ' + (SIMPLE_SHADERS ? 'simple' : 'full') + ' shaders. Changing this reloads the game.'; $('#shSel').onchange = () => { const v = $('#shSel').value; try { if (v === 'auto') localStorage.removeItem('vg_shaders'); else localStorage.setItem('vg_shaders', v); } catch (e) { } location.reload(); }; }
