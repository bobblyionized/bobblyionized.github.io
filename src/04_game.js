...
function startSpike() {
  ...
  else if (dc) { P.chargeStart = T - 0.1875; P.charge = 0.5; }                    // fresh off a dash: the bar begins at 50%
...
function tryDash() {                          // Dash (ability trait): a burst in your movement direction, 3 s cooldown; in the air it kills vertical momentum and refreshes your air action
  ...
  P.dash = { t0: T, dur: 0.15, dir }; P.dashReady = T + 5; P.vel.set(0, 0, 0); P.glide = null; P.dashCharge = true;   // the next spike after a dash starts half charged
...
function updatePlayer(dt) {
  ...
  if (P.dash) {
    ...
    else { const sp = 46.2 * (1 - e * 0.55); P.vel.x = P.dash.dir.x * sp; P.vel.z = P.dash.dir.z * sp; if (!P.onGround) P.vel.y = 0; if (e > 0.1 && Math.random() < 0.7) sparkle(P.pos.clone().add(new V3(0, 0.8, 0)), 3, 0xffe066, 0.35, 0.3, 0.25, 0.05); }   // yellow trail
  } else if (P.dive) {
